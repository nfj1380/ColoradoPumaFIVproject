...
WD <- "/Users/melanielacava/Data/MD/LGanalysis"
setwd(WD)
...
# library(fossil)
# coords<-read.csv("LatLong_406ind.csv", header=F)
# lat<-coords[,2:3]
# geo.dist<-earth.dist(lat, dist=F)
# #remove top half of matrix (including diagonal)
# geo.dist[upper.tri(geo.dist, diag=TRUE)] <- NA
# #export
# save(geo.dist,file=paste(getwd(),"/geo.dist.rData",sep=""))
load("geo.dist.rData")

## Genetic distance 
# WD <- "/Users/melanielacava/Data/MD/"
# setwd(WD)
# library(PopGenReport)
# #calc dist - takes a while (~30-60 min)
# #md406 <- read.structure("md_dp3_maf0.01_miss0.6_ind0.6.stru", n.ind=406, n.loc=4069, onerowperind=F, col.lab=1, col.pop=2, ask=F)
# #smouse.dist.406 <- gd.smouse(md406, verbose=TRUE)
# #write.csv(as.matrix(smouse.dist.406), "SPgendistance_md406.csv")
# #OR import genetic distance matrix after making
# gen.dist <- as.matrix(read.table("SPgendistance_md406.csv", sep=",", header=T)[,-1])
# rownames(gen.dist) <- colnames(gen.dist)
# gen.dist[1:10,1:10] #check
# #remove top half of matrix (including diagonal)
# gen.dist[upper.tri(gen.dist, diag=TRUE)] <- NA
# save(gen.dist,file=paste(getwd(),"/gen.dist.rData",sep=""))
load("gen.dist.rData")
...
file.list <- list.files("/Users/melanielacava/Data/MD/LGanalysis/RastersToTest", pattern=".tif",full.names = T)
file.nms <- list.files("/Users/melanielacava/Data/MD/LGanalysis/RastersToTest/", pattern=".tif",full.names = F)
file.nms <- gsub("\\..*","",file.nms )

# set output directory
out.dir<-"/Users/melanielacava/Data/MD/LGanalysis/TransformedRasters/"
...
x <- raster("/Users/melanielacava/Data/MD/LGanalysis/TransformedRasters/elev_buf50k_res180m_positive_linear.tif")
plot(x)
...
#STEP 3 NEEDS TO HAPPEN ON TETON
...
in.dir <- "/Users/melanielacava/Data/MD/LGanalysis/TransformedRasters"
res.files <- list.files(in.dir, pattern=".tif", full.names = T)

# specify out dir
out.dir <- "/Users/melanielacava/Data/MD/LGanalysis/ResistanceDistances/"
...
## get file list
#in.dir<-"/Users/melanielacava/Data/MD/LGanalysis/ResistanceDistances/"
in.dir<-"/Users/melanielacava/Data/MD/LGanalysis/FromTeton/"
...
## write out
write.csv(df, "/Users/melanielacava/Data/MD/LGanalysis/Univariate_MLPE_AICc_Results_Interstates90m.csv",row.names=F)
